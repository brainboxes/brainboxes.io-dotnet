...
namespace BBDemoApp
{
    class BBDemoApp
    {
        ...
        private static readonly ILog logger = LogManager.GetLogger(typeof(BBDemoApp));

        public BBDemoApp(int listenOnPort = 8888, int webSocketListenOnPort = 8989)
        {
            logger.Info("Starting servers");
            httpServer = new HttpServer(listenOnPort);
            socketServer = new BBWebSocketServer(webSocketListenOnPort);
            //remoteIO = new RemoteIO("YOUR_DEVICE_IP", "YOUR_DEVICE_IP");
        ...
        private void HttpAction(HttpListenerContext context)
        {
            ...
            logger.Info("New HTTP Request:\t" + path + "\t" );
        ...
        {
            logger.Info("Sending Current ED State");
